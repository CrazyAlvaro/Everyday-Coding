...
def preprocessor(df_ego, df_obj, ego_obj_id, shift_intrplt_cols, _verbose=False):
    ...
    if _verbose:
        print("========= preprocessor ===========")
        df_ego_augment.to_csv('debug/ego_00_augment.csv', index=False)
...
def ego_interpolate(df_ego_interpolated, time_intrplt_cols, shift_intrplt_cols, ego_config, _verbose=False):
    ...
    if _verbose:
        ...
        df_ego_interpolated.to_csv('debug/ego_0_augment_interpolated.csv', index=False)
...
def obj_augment(df_obj, df_ego_interpolated, ts_ego, obj_time_cols, obj_shift_cols, _verbose=False):
    ...
    if _verbose:
        df_output.to_csv('debug/obj_1_augment.csv', index=True)
...
def _calculate_track_statistics(df_tracks, label_system):
    """
    ...
    for track_id, group in tqdm(df_tracks.groupby('trackId'), desc="Tracks Statistics: "):
        ...
        track_stats.append(_curr_track)
...
def reference_matching(df_obj_augment, ego_obj_id, columns_tracks, _verbose=False):
    ...
    if _verbose:
        df_obj_augment.to_csv('debug/obj_2_before_first_trav.csv')
    ...
    # Drop columns start with "OBJ"
    df_obj_augment = df_obj_augment.drop(columns=[col for col in df_obj_augment.columns if 'OBJ' in col])

    if _verbose:
        df_obj_augment.to_csv('./debug/obj_3_after_first_trav.csv')
...
def track_data_generator(df_obj_augment, ego_timestamps, cols_tracks, cols_recording_meta, frame_rate, label_system=True, _verbose=False):
    ...
    pd_tracks_meta = _calculate_track_statistics(pd_tracks, label_system)
...
def args_handler():
    ego_file = "data/ego.csv"
    obj_file = "data/obj.csv"
...
def tracks_generator(case_name, ego_file, obj_file, ego_config_file):
    """
    ...
    print("==================================================================================\n")
    print(" 5 Steps: Obj_Augment, Reference_Matching, Checking_Surroundings\n")
    print("==================================================================================\n")
